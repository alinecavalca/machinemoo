...
N_SENTENCES_RANDOM = (
    3  # number of sentences to use for building random chunks when not bias-related
)
        ...
        for b in bias:
            if b in sentence:
                ...
                bias_sentences.append(
                    " ".join([sent for sent in sentences[idx_start:idx_end]])
                )
...
def full_extractor(text):
    ...
    while len(sentences) < N_CHUNKS:
        ...
        )  # we pad the sentences list by repeating random sentences
...
for idx, row in df_annotation.iterrows():
    ...
    json_annotation[row["nome_arquivo"]].update(
        {
            k: ""
            for k, v in json_annotation[row["nome_arquivo"]].items()
            if type(v) is float
        }
    )
    ...
    json_annotation[row["nome_arquivo"]].update(
        {
            k: v.split(";")
            for k, v in json_annotation[row["nome_arquivo"]].items()
            if type(v) is str and k != "nome_arquivo"
        }
    )
