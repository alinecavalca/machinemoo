...
def clean_text(text):
    ...
    # .AA
    text = re.sub(
        r"(\.)(?=[A-ZÁÀÃÄÂÉÈẼËÊÍÌĨÏÓÒÕÖÔÚÙŨÜÇŸ][A-ZÁÀÃÄÂÉÈẼËÊÍÌĨÏÓÒÕÖÔÚÙŨÜÇŸ])",
        "!PERIOD! ",
        text,
    )
    # .Aa
    ...
    text = re.sub(
        r"[^A-Za-z0-9 \?!,;\-ÁÀÃÄÂÉÈẼËÊÍÌĨÏÓÒÕÖÔÚÙŨÜÇŸáàãäâéèẽëêíìĩïóòõöôúùũüçÿ]+",
        "",
        text,
    )
    ...
    text = re.sub(
        r"(?<=[A-ZÁÀÃÄÂÉÈẼËÊÍÌĨÏÓÒÕÖÔÚÙŨÜÇŸ])(?=[A-ZÁÀÃÄÂÉÈẼËÊÍÌĨÏÓÒÕÖÔÚÙŨÜÇŸ][a-záàãäâéèẽëêíìĩïóòõöôúùũüçÿ])",
        " ",
        text,
    )
    ...
    text = re.sub(
        r"(?<=[A-ZÁÀÃÄÂÉÈẼËÊÍÌĨÏÓÒÕÖÔÚÙŨÜÇŸ])(?=(RELATORA|RELATOR))", " ", text
    )
    ...
    text = re.sub(
        r"Poder Judici(á|a)rio Tribunal de Justi(ç|c)a do Estado de S(ã|a)o Paulo",
        "",
        text,
    )
    text = re.sub(
    ...
    text = re.sub(
        r"Tribunal de Justi(ç|a)a Poder Judici(á|a)rio S((ã|a)o Paulo|(Ã|A)O PAULO)",
        "",
        text,
    )
    text = re.sub(
        r"TRIBUNAL DE JUSTI(Ç|C)A PODER JUDICI(Á|A)RIO S((ã|a)o Paulo|(Ã|A)O PAULO)",
        "",
        text,
    )
    # removes 'fl' and 'fls'
    ...
    # remove assinatura
    text = re.sub(
        r"(Assinatura( |)Eletr(ô|o)nica|ASSINATURA( |)ELETR(Ô|O)NICA)", "", text
    )
    text = re.sub(
        r"(\bEste documento é cópia do original, assinado digitalmente por.*?autos em (à|a)s\b)",
        "",
        text,
    )
...
def text_extractor(path):
    ...
    for file in os.listdir(directory):
        ...
        if filename.endswith(".pdf"):
            ...
            text_filename = (
                filename[: len(filename) - 4] + ".txt"
            )  # replaces .pdf extension for .txt / substitui extensao .pdf por .txt
