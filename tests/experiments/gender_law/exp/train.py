...
import torch
from model import BertBaseline, BertFineTuner, GBVDataset
...
def prepare_datasets(json_path: str):
    ...
    train_keys, test_keys = train_test_split(
        list(data.keys()), test_size=0.1, random_state=42
    )
...
def run(args):
    ...
    trainset = GBVDataset(train_data, tokenizer, args.max_seq_length, data_type="train")

    valset = GBVDataset(val_data, tokenizer, args.max_seq_length, data_type="val")

    testset = GBVDataset(test_data, tokenizer, args.max_seq_length, data_type="test")

    trainloader = DataLoader(trainset, batch_size=args.batch_size, shuffle=True)
    valloader = DataLoader(valset, batch_size=args.batch_size, shuffle=False)
    testloader = DataLoader(testset, batch_size=args.batch_size, shuffle=False)

    model = BertFineTuner(model_name=args.model_name, num_classes=args.num_classes)
    ...
    scheduler = CosineAnnealingLR(
        optimizer,
        T_max=len(trainset) // args.batch_size * args.n_epochs,
        eta_min=1e-8,
        verbose=True,
    )
    ...
    def train_step(input):
        ...
        labels_one_hot = torch.nn.functional.one_hot(
            labels, num_classes=args.num_classes
        ).to(device)
    ...
    def test_step(input):
        ...
        if (
            1 in y_hat
        ):  # if bias was detected in any chunk, all chunks from the same case are assigned the same label
    ...
    for epoch in tqdm(range(args.n_epochs), leave=True):
        ...
        for input in tqdm(trainloader, desc=f"Training Epoch {epoch}", leave=False):
            optimizer.zero_grad()
            train_loss_batch, train_bal_acc_batch, y_true_batch, y_pred_batch = (
                train_step(input)
            )
            optimizer.step()
        ...
        print(
            f"Epoch {epoch} - Train Loss: {train_loss_epoch:.4f} - Train Balanced Accuracy: {train_bal_acc_epoch:.4f}"
        )
        y_true_train = [
            item for sublist in y_true_train for item in sublist
        ]  # flattens lists of y_true
        y_pred_train = [
            item for sublist in y_pred_train for item in sublist
        ]  # flattens lists of y_pred
        if (
            train_loss_epoch < current_loss_train
        ):  # if we are in a better epoch than before, lets build a confusion matrix
        ...
        for input in tqdm(valloader, desc=f"Validation Epoch {epoch}", leave=False):
            optimizer.zero_grad()
            valid_loss_batch, valid_bal_acc_batch, y_true_batch, y_pred_batch = (
                validation_step(input)
            )
            optimizer.step()
        ...
        print(
            f"Epoch {epoch} - Validation Loss: {valid_loss_epoch:.4f} - Validation Balanced Accuracy: {valid_bal_acc_epoch:.4f}"
        )
        y_true_val = [
            item for sublist in y_true_val for item in sublist
        ]  # flattens lists of y_true
        y_pred_val = [
            item for sublist in y_pred_val for item in sublist
        ]  # flattens lists of y_pred
        if (
            valid_loss_epoch < current_loss_val
        ):  # if we are in a better epoch than before, lets build a confusion matrix
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--device", type=str, default="cpu", help="GPU ID to run the model"
    )
    parser.add_argument(
        "--no_cuda",
        type=bool,
