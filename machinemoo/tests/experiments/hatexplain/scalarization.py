...
class MultiTaskModel(nn.Module):
    def __init__(self, task_names, input_dim=50, hidden_dim=32, num_labels=2, dropout_rate=0.3):
        ...
        self.classifiers = nn.ModuleDict({
            task: nn.Linear(hidden_dim, num_labels)
...
class NLPScalarization(Scalarization):
    def __init__(self, train_dataloader, device, task_names: list[str], gradient: bool = False):
    ...
    def get_gradients(self, model: nn.Module, loss: torch.Tensor, layer: Optional[nn.Module] = None) -> np.ndarray:
    ...
    def compute_task_gradients(self, batch: dict, criterion: nn.Module) -> np.ndarray:
        self.model.eval()
    ...
    def training(self, weight):
        ...
        for epoch in range(self.epochs):
            ...
            print(f"Epoch {epoch+1} - Average loss: {total_loss / len(self.train_dataloader):.4f}")
