...
class GBVDataset(Dataset):
        ...
        for item in data.values():
            for text in item[chunk_type]:
                ...
                if class_field == "vies":
                    ...
                    print(
                        "Class codes for other attributes have yet to be implemented."
                    )
                    exit()
    ...
    def encode(self, texts, tokenizer):
        ...
        return (
            [encode["input_ids"] for encode in encodes],
    ...
    def __getitem__(self, idx):
        ...
        labels = (
            self.labels[idx] if type(self.labels[idx]) is list else [self.labels[idx]]
        )
...
class BertFineTuner(torch.nn.Module):
    ...
    def __init__(self, model_name, num_classes):
        ...
        self.model = AutoModelForSequenceClassification.from_pretrained(
            model_name, num_labels=num_classes, cache_dir="."
        )
        ...
        for layer in self.model.bert.encoder.layer[
            :-5
        ]:  # freezes all but last 5 layers -- fine tuning is performed over them
...
class BertBaseline(torch.nn.Module):
    ...
    def __init__(self, model_name, num_classes):
        ...
        ):  # freezes the whole network -- fine tuning is performed over last layer only (classifier)
            for param in layer.parameters():
                param.requires_grad = False
