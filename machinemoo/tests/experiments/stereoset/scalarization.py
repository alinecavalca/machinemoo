...
class MultiTaskModel(nn.Module):
    def __init__(
        self, task_names, input_dim=50, hidden_dim=32, num_labels=2, dropout_rate=0.3
    ):
...
class NLPScalarization(Scalarization):
    def __init__(
        self, train_dataloader, device, task_names: list[str], gradient: bool = False
    ):
    ...
    def get_gradients(
        self, model: nn.Module, loss: torch.Tensor, layer: Optional[nn.Module] = None
